...
While the Mexican hat function can be seen as an $L2$ regularization equivalent penalizer for binary values, here, we choose an $L1$-equivalent form.
Specifically, we choose $\phi(X)=\sum_{i,j}\Lambda(X_{ij})$, which employs the one-dimensional function 
\[
	\Lambda(x) = 
    \begin{cases}
        -|1-2x|+1 &x\in[0,1]\\
...
We derive with the following proposition a closed form for the computation of the exact minimum as assigned by the proximal mapping with respect to $\phi$. 
\begin{theorem}
Let $\alpha>0$ and $\phi(X)=\sum_{i,j}\Lambda(X_{ij})$ for $X\in\R^{m\times n}$. The proximal operator of $\alpha\phi$ maps the matrix $X$ to the matrix $\prox_{\alpha\phi}(X)=A\in [0,1]^{m\times n}$ defined by $A_{ji}=\prox_{\alpha\Lambda}(X_{ji})$, where for $x\in \R$ it holds that
...
%\qed
\end{proof}
The regularization with $\phi$ meets the requirements of the nonsmooth part in an optimization by PALM. 
\begin{lemma}
The function $\phi(X)=\sum_{i,s}\Lambda(X_{is})$ is proper, lower semi-continuous and semi-algebraic function.
\end{lemma}
\begin{proof}
...
Similarly, the semi-algebraic property of $\phi$, being a finite sum of function values of $\Lambda$, follows from the semi-algebraic property of $\Lambda(x)$. We write $\Lambda(x)=\delta_{[0,1]}(x)|1-(1-2x)|$, where $\delta_{[0,1]}(x)$ is the indicator function, returning infinity if $x$ is outside of the interval $[0,1]$ and one otherwise. The function $|1-(1-2x)|$ is semi-algebraic as a composition of a polynomial function and the one-norm. The indicator function $\delta_{[0,1]}(x)$ is semi-algebraic if the interval $[0,1]$ is a semi-algebraic set. This is indeed the case according to the definition of semi- algebraic sets, since the interval $[0,1]=\{x\mid p(x)<0\}\cup\{x\mid p(x=0)\}$ for the polynomial function $p(x)=(x-0.5)-0.25$.
\end{proof}
%--------------------------------------
% Minimizing the Approximation Error
%--------------------------------------
\subsection{Minimizing the Approximation Error}
 We state the functions, gradients and Lipschitz constants, required for the application of PALM for the optimization of problem \eqref{eq:BoolMF}. The relaxed objective is the one which we intend to optimize with PALM.
\begin{mybox}
Optimization of a Boolean matrix factorization via PALM.
\begin{align*}
    F_{BMF}(X,Y) &= |D-\theta(YX^\top)| & \textit{(objective)}\\
    \Psi(X,Y)&=\|D-YX^\top\|^2 +\phi(X)+\phi(Y)& \textit{(relaxed objective)}\\
    F(X,Y)&=\|D-YX^\top\|^2 &\textit{(smooth part)}\\
...
\end{align*}
\end{mybox}
The function $\phi_B$ is the nonbinary penalization function, which we define in the following Section. The stated Lipschitz-moduli are easily derived via the inequality
\begin{align*}
...
The proposed approach has applications to the general problem of matrix factorizations under binary constraints. \cite{shen2016fast} propose to find optimal hashing codes via PALM, where the matrix denoting the hash codes is strictly constrained to binary values. That is, the proximal mapping thresholds the gradient updates of the code matrix to binary values after every gradient descent update. Such a procedure is likely to get stuck in local optima, reflecting less suitable solutions. 