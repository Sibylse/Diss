...
We employ the framework \emph{PAL-Tiling} to optimize a description length having a smooth, real-valued approximation, suitable for the optimization via PALM.\todo{main contributions instead} The built-in regularization of the model complexity by MDL is used to derive factorizations which do not use more tiles than necessary. We propose two algorithms as specifications of \textsc{PAL-Tiling}: one applying $\ell 1$-regularization on the matrix factorization (\textsc{Panpal}) and one employing an encoding by code tables as proposed by \cite{siebes2006item} (\textsc{Primp}). 
...
We assume w.l.o.g.\@ that $X,X^*\in\{0,1\}^{n\times r}$ and $Y,Y^*\in\{0,1\}^{n\times r}$, otherwise we attach zero columns to the matrices such that the dimensions match. 
