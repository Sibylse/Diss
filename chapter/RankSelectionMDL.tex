...
%===================================
% Minimizing the Description Length in PAL-Tiling
%===================================
...
\end{restatable}
The proof of Lemma~\ref{thm:CTBMF} can be found in Appendix~\ref{chap:AppendixMDL}.
...
\begin{algorithmic}[1]
  \Function{GenerateNoisyBMF}{$n,m,r^*,\xi_{max},p_+^*,p_-^*$}
  	\State $X^*\gets$ \Call{UniformRand}{$\left\{X\in\{0,1\}^{n\times r}\middle\vert 0.01\leq\frac{\lvert X_{\cdot s}\rvert}{n}\leq \xi_{max}\right\}$}
  	\State $Y^*\gets$  \Call{UniformRand}{$\left\{Y\in\{0,1\}^{m\times r}\middle\vert 0.01\leq\frac{\lvert Y_{\cdot s}\rvert}{m}\leq \xi_{max}\right\}$}
